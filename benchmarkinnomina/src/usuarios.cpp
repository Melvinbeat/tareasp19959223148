...
bool usuarios::loginUsuarios(){
    ...
    do{
        ...
        if (usuario==USER && contra==PASS){ //ambos deben coincidir y antes se revisan peros son CONSTANTES
            ingresa=true;
            //bitacora.setRegistro(usuario, "Login correcto");
        } else {
            cout<<"\nEl usuario y/o contrasena son incorrectos"<<endl;//si no coinciden se envia el mensaje de error
            cin.get();
            contador++;
